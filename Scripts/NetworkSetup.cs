...
public partial class NetworkSetup : Control
{
    ...
    {
        Logger.Debug($"Peer connected: {id}");
        if (id == 1)
    ...
    {
        Logger.Debug($"Peer disconnected: {id}");
    ...
    public void OnConnectPressed()
    {
        ...
        {
            Logger.Error("Need a remote to connect to.");
            return;
        }
    ...
    {
        Logger.Debug("Calling ChangeLevel");
        RemoveOldLevel();
    ...
    {
        Logger.Debug("Nickname requested");
    ...
    {
        Logger.Debug("Nickname received: " + name);
    ...
    {
        Logger.Debug($"Registering player with id {id} and name {name}");
        var isHost = id == 1;
    ...
    {
        Logger.Debug($"Adding remote player with id {id} and name {name}");
        var isHost = id == 1;
        Players.Add(new Player { Id = id, Name = name, Host = isHost, Ai = false });
        UpdatePlayersList();
    }
