...
/// <summary>
/// Sprache parsers for the card-action DSL used in deck YAML.
/// </summary>
public static class ActionParser
{
   ...
   /// <summary>
   /// Parses a numeric or comparison expression such as <c>self.Tower &lt; opponent.Tower</c>.
   /// Used by YAML <c>if:</c> conditions. Additive operator is <c>+</c>; multiplicative is <c>*</c>.
   /// </summary>
   ...
   private static readonly Parser<Expression> _operand = _number.Or(_variable).Or(_parenthesizedExpression);
   ...
   private static readonly Parser<Expression> _expression = _comparisonExpression.Or(_additiveExpression);
   ...
   {
      if (idList.Count < 2)
         return Fail<(TargetType Target, ResourceTypes? Resource, EffectType Method)>("Expected at least two identifiers for method call");
   ...
   /// <summary>
   /// Parses a single scalar action such as <c>self.Tower.Gain(10)</c> or an inline
   /// <c>if … then … else …</c>. YAML mapping conditionals are handled by <see cref="ActionTypeConverter"/>.
   /// </summary>
   public static readonly Parser<ActionBase> Action = _conditional.Select(ActionBase (a) => a)
